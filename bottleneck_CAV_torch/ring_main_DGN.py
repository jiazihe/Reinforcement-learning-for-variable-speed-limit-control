...
# total 22  (av + human)
agent_num = 5   # number of autonomous driving  (rl)
neighbors = 5   # 
n_ant = agent_num

## define some environment parameters
exp_tag = "dgn_ring"
result_dir = mkdir('{}_results'.format(exp_tag))
...
if MODE == 'train':
    ...
    model.load_state_dict(torch.load('./dgn_ring_results/model_100'))
...
for i_episode in range(num_runs):
    ...
    outflows = []
    comp_cnt = 0
    # for time space graph
    lane_pos = {}
    cur_distance = {}
    all_ids = [vel_id for vel_id in env.k.vehicle.get_ids()]
    ...
    for j in range(num_steps):
        ...
        for i in range(n_ant):
            if np.random.rand() > epsilon:
                ...
                a = q[i].argmax().item()
                aset_arg[i] = a
                action_lists = [-1,-0.8,-0.7,-0.6, -0.5, 0.3, 0.4, 0.5, 1]    # important
                a = action_lists[a]
                aset[i] = a
    ...
    throughput.append(sum(outflows) / len(outflows)) 
    OUTFLOWS.append(outflows)
    ARRIVE.append(arrive)   ## collision rate   (crash) / total
    ...
    np.save(os.path.join(result_dir,'ES_Total_scores.npy'), throughput)
    ...
    np.save(os.path.join(result_dir,'target_hw.npy'), TARGET_HW)
    np.save(os.path.join(result_dir,'TIME_hw.npy'), TIME_HW)
    ...
    if MODE == 'train':
        ...
        # update NN parameters (if test not update)
